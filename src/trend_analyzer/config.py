...
class Config:
    ...
    def __init__(self):
        ...
        # Load config files
        self.infrastructure_config, self.analysis_config, self.dimensions_config = (
            load_config_files()
        )
        ...
        debug(
            f"OpenAI API Key loaded: {'set' if self.OPENAI_API_KEY != 'placeholder-key' else 'placeholder'}"
        )
        ...
        if self.analysis_config:
            ...
            info(
                f"Enabled operations: {', '.join(operations) if operations else 'none'}"
            )
